...
return ( <Link to={item.path} onClick={onClick} className="relative group"> <span className="group-hover:text-indigo-600 transition">
{item.name} </span> <span className="absolute left-1/2 -bottom-2 w-0 h-0.5 bg-indigo-600 transition-all group-hover:w-full group-hover:left-0"></span> </Link>
);
}
...
function HeaderCTA() {
return ( <div className="hidden md:flex items-center gap-4"> <Link
     to="/contact"
...
/>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <button onClick={() => setOpen(false)} className="text-gray-600 text-lg">
            ✕
          </button>
        </div>
...
return ( <header className="w-full bg-white fixed top-0 left-0 z-50 border-b border-gray-200"> <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">


    {/* Logo */}
    <Link to="/" className="flex items-center gap-3">
      <img src={Logo} alt="Poeage Logo" className="h-16 object-contain" />
    </Link>
